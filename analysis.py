...
def compute_largest_cc_size(g):
    ...
    nodes = list(g.keys())  # make a copy of all the keys in dictionary
    for node in nodes: #check each node
        Q.append(node) #put it in que
        nodes.remove(node) #remove a node that has been checked from the list
...
def random_attack(g,percentage):
    ...
    """
    nodes_removed = 0
    dict_g = {0: compute_largest_cc_size(g)}
    remove_nodes = round(percentage * len(g))

    for _ in range(remove_nodes):

        i = list(g.keys())
        rand_node = random.choice(i)
        g.pop(rand_node)
        ...
        nodes_removed += 1
        dict_g[nodes_removed] = compute_largest_cc_size(g)
...
def target_attack(g,percentage):
    ...
    dict_target = {0: compute_largest_cc_size(g)}
    remove_nodes = round(percentage * nodes)

    for _ in range(remove_nodes):

        nodes_list = list(g.keys())
        
        C = [len(v) for v in g.values()] # degree of each node
        max_idx = C.index(max(C)) # index of node with largest degree
        max_node = nodes_list[max_idx]
        
        g.pop(max_node) # remove node with largest degree

        for v in g.values(): # remove edges of node with largest degree
               if max_node in v:
                v.remove(max_node)

        nodes_removed += 1

        dict_target[nodes_removed] = compute_largest_cc_size(g)
    list_target = [dict_target]
    return list_target
